...
export default function FirstAccessCollaborator() {
  ...
  const [userData, setUserData] = useState<{ name: string; email: string; userId: string; roles: string[] } | null>(null);
  ...
  const validateToken = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          profiles!activation_tokens_user_id_fkey (
            full_name,
            email
          )
        `)
      ...
      if (error || !data) {
        ...
        const { data: rolesData } = await supabase
          .rpc('get_user_roles', { _user_id: data.user_id });

        const roleNames = rolesData?.map((r: any) => r.role_name) || [];
  ...
  const handleValidateAccount = async (e: React.FormEvent) => {
    ...
    try {
      // Update user password using admin API (through service role)
      ...
      toast({
        ...
        description: "Agora você pode fazer login para acessar o sistema.",
      });
        ...
        {success ? (
          ...
          <div className="flex flex-col items-center gap-6 py-4">
            ...
            <div className="space-y-2 text-center">
              ...
              <p className="text-muted-foreground">
                Sua senha foi criada com sucesso. Agora você pode fazer login.
              </p>
            </div>
            <Button
              onClick={handleLogin}
          ...
          <>
            <div className="space-y-2 text-center">
              ...
              <p className="text-muted-foreground">
                Bem-vindo(a), <span className="font-semibold text-primary">{userData?.name}</span>! 
                Crie uma senha para acessar o sistema.
              </p>
              {userData?.roles && userData.roles.length > 0 && (
                <p className="text-sm text-muted-foreground">
                  Função: <span className="font-medium text-foreground">{userData.roles.join(", ")}</span>
                </p>
              )}
            </div>
